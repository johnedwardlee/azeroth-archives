...
export function JournalManager({ character, patchCharacter }: { character: CharacterData; patchCharacter: (patch: Partial<CharacterData>) => void }) {
  ...
  const update = (id: string, patch: Partial<JournalEntry>) => patchCharacter({ journal: character.journal.map((entry) => entry.id === id ? { ...entry, ...patch, updatedAt: new Date().toISOString() } : entry) });
  const add = () => {
    const now = new Date().toISOString();
    patchCharacter({ journal: [{ id: crypto.randomUUID(), type: newType, title: `New ${labels[newType]}`, details: "", status: "active", pinned: false, createdAt: now, updatedAt: now }, ...character.journal] });
  };
  return <div className="journal-layout">
    <section className="panel journal-toolbar">
      <div className="section-heading"><div><span className="eyebrow">Campaign memory</span><h2>Journal</h2></div><BookOpen size={20} /></div>
      ...
      <label className="legacy-notes"><span>Scratchpad</span><textarea value={character.notes} onChange={(event) => patchCharacter({ notes: event.target.value })} placeholder="Quick notes that do not need a structured entry" rows={7} /></label>
    </section>
    <section className="journal-entry-list">
      {visible.map((entry) => <article className={`panel journal-entry status-${entry.status}`} key={entry.id}>
        <div className="journal-entry-heading"><span>{entry.type === "npc" ? <Users size={13} /> : entry.type === "location" ? <MapPin size={13} /> : entry.status === "completed" ? <Check size={13} /> : <BookOpen size={13} />}{labels[entry.type]}</span><div><button className={entry.pinned ? "pinned" : ""} aria-label={`${entry.pinned ? "Unpin" : "Pin"} ${entry.title}`} onClick={() => update(entry.id, { pinned: !entry.pinned })}><Star size={14} /></button><button className="danger" aria-label={`Remove ${entry.title}`} onClick={() => patchCharacter({ journal: character.journal.filter((item) => item.id !== entry.id) })}><Trash2 size={14} /></button></div></div>
        <input className="journal-title" value={entry.title} onChange={(event) => update(entry.id, { title: event.target.value })} />
        <textarea value={entry.details} onChange={(event) => update(entry.id, { details: event.target.value })} placeholder="Names, discoveries, goals, and outcomes" rows={6} />
        <select value={entry.status} onChange={(event) => update(entry.id, { status: event.target.value as JournalEntryStatus })}><option value="active">Active</option><option value="completed">Completed</option><option value="archived">Archived</option></select>
      </article>)}
