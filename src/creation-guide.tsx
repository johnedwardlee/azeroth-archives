...
import { ABILITY_LABELS, type AbilityKey, type AdvancementChoice, type BackgroundDefinition, type CharacterData, type EquipmentDefinition, type FeatDefinition, type InventoryItem } from "../lib/types";
...
export function CreationGuide({ character, patchCharacter, background, feats, equipment }: { character: CharacterData; patchCharacter: PatchCharacter; background?: BackgroundDefinition; feats: FeatDefinition[]; equipment: EquipmentDefinition[] }) {
  ...
  return <section className="panel creation-guide-panel">
    ...
    <div className="guide-sections">
      <div className="guide-section">
        ...
        <select aria-label="Ability score method" value={character.abilityScoreMethod} onChange={(event) => chooseAbilityMethod(event.target.value as CharacterData["abilityScoreMethod"])}><option value="standard-array">Standard array</option><option value="point-buy">Point buy</option><option value="rolled">Rolled scores</option><option value="manual">Manual / GM assigned</option></select>
      ...
      {startingEquipment && <div className="guide-section"><strong>Starting equipment</strong><p>{startingEquipment}</p><div className="starting-equipment-actions"><button className={character.startingEquipmentChoice === "A" ? "selected" : ""} onClick={() => applyStartingEquipment("A")}>Add package A</button><button className={character.startingEquipmentChoice === "B" ? "selected" : ""} onClick={() => applyStartingEquipment("B")}>Take option B gold</button></div><label className="guide-confirm"><input type="checkbox" checked={character.startingEquipmentConfirmed} onChange={(event) => patchCharacter({ startingEquipmentConfirmed: event.target.checked })} />Equipment reviewed; allow GM-approved substitutions</label></div>}
