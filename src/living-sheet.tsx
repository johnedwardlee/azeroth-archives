...
export function SpellbookManager({ catalog, equipmentCatalog, character, patchCharacter, spellcastingProfiles, creationLocked = false, showCreationSetup = true, onRoll }: { catalog: SpellDefinition[]; equipmentCatalog: EquipmentDefinition[]; character: CharacterData; patchCharacter: PatchCharacter; spellcastingProfiles: SpellcastingProfile[]; creationLocked?: boolean; showCreationSetup?: boolean; onRoll?: (event: LocalRollEvent) => void }) {
  ...
  function rollSpellEffect(spell: SpellDefinition, formula: string, category: LocalRollEvent["category"], modifier = 0, instances = 1) {
    const results = Array.from({ length: instances }, () => rollDiceFormula(formula, false, modifier));
    if (results.some((result) => !result)) return;
    const resolved = results.flatMap((result) => result ? [result] : []);
    const dice = resolved.flatMap((result) => result.rolls);
    const totalModifier = resolved.reduce((sum, result) => sum + result.modifier, 0);
    const total = resolved.reduce((sum, result) => sum + result.total, 0);
    setSpellRollResult(`${spell.name}: ${dice.join(" + ")}${totalModifier ? ` ${totalModifier >= 0 ? "+" : "−"} ${Math.abs(totalModifier)}` : ""} = ${total}`);
    onRoll?.({ category, label: spell.name, formula: `${instances > 1 ? `${instances}×` : ""}${formula}`, dice, modifier: totalModifier, total, mode: "normal", detail: category === "healing" ? "Hit Points restored" : "Spell effect" });
  }
    ...
    <div className="living-tab-grid spellbook-layout">
      ...
      <CollapsiblePanel className={`spell-list-panel ${showCreationSetup ? "" : "hide-creation-setup"}`} storageKey={`azeroth-panel-${character.id}-spellbook`} eyebrow="Known magic" title="Spellbook" summary={<span>{character.spells.length} known · {character.spells.filter((spell) => spell.prepared).length} prepared</span>}>
        ...
        <div className="spell-card-list">{visibleSpells.map((spell) => {
          ...
          const effectLevel = selectedCastLevel ?? spell.level;
          const healingProfile = spellHealingProfile(spell, effectLevel);
          const damageProfile = spellDamageProfile(spell, effectLevel, character.level);
          const genericFormula = extractDiceFormula(spell.description);
          const healingModifier = healingProfile?.addsSpellcastingModifier && owner ? abilityModifier(character.abilities[owner.ability]) : 0;
